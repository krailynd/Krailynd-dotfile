...
<!-- gentle-ai:visual-eyes -->
## Visual Eyes Protocol (MANDATORY)

You have multimodal vision. USE IT — never answer visual questions from memory or code alone.

Trigger phrases (any language): "mira esto", "look at this", "revisa este diseño", "review this design", "analiza esta interfaz", "analyze this interface", "captura", "screenshot", "cómo se ve", "how does it look", "enséñame la página", "muéstrame el diseño".

When the user asks you to LOOK at something visual:

1. **URL given or implied** (site, docs page, deployed app): use the `playwright` MCP tools — `browser_navigate` to the URL, then `browser_screenshot` (full page, reasonable viewport like 1280x800) — and answer from the captured image with full multimodal context.
2. **Local file** (HTML, image, PDF, Figma export, design file): `Read` the file directly. Images and PDFs are returned as visual attachments — inspect them, then describe/analyze.
3. **Running app on localhost** (dev server, preview port): navigate to the local URL first (e.g. `http://localhost:5173`), then screenshot.
4. **Other people's screenshots / pasted images**: never describe blindly — analyze the actual pixels you receive and reference concrete elements.
5. **Base every answer on what the screenshot/file actually shows**: name layout, colors, spacing, typography, copy, and concrete UI issues. NEVER invent details that are not visible in the capture.
6. **Ambiguous target**: ask exactly ONE question (URL? file? app?) — or, if one target is clearly most likely, capture it and confirm.

Screenshots are evidence, not decoration: cite what you see (regions, elements, colors) when giving design or UI feedback.
<!-- /gentle-ai:visual-eyes -->
