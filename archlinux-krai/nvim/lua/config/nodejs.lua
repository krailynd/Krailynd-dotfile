    ...
    if vim.v.shell_error == 0 then
      ...
      if version then
        ...
        if major_version and major_version >= 18 then
          -- Provider host must be neovim npm CLI (cli.js), never the node binary itself.
          ...
          local cli_candidates = {
            ...
            vim.fn.expand("~/.npm-global/lib/node_modules/neovim/bin/cli.js"),
          }
          local cli_found = false
          for _, p in ipairs(cli_candidates) do
          ...
          if not cli_found then
            -- Let provider auto-detect via `npm root -g`; do not pollute host with node binary.
            vim.g.node_host_prog = nil
            -- To silence the resulting WARN when you don't use Node plugins:
          ...
          -- Do not set node_host_prog to node binary (causes health error); leave for provider detection.
          vim.g.node_host_prog = nil
          return true, version
        end
      else
...
local function check_node_version()
  -- Check the actual node binary, not the neovim host CLI (which is a .js file).
...
function M.info()
  local node_bin = vim.fn.exepath("node")
  if node_bin ~= "" then
    local vout = vim.fn.system(node_bin .. " --version 2>/dev/null")
    if vim.v.shell_error == 0 then
      local v = vout:gsub("\n", ""):gsub("v", "")
      print("Node.js binary: " .. node_bin .. " (v" .. v .. ")")
    end
  end
  if not vim.g.node_host_prog then
    print("Node host: not set (provider will auto-detect via npm root -g)")
    if vim.g.loaded_node_provider == 0 then
      print("Node provider disabled (g:loaded_node_provider=0)")
    end
    return
  end

  local host = vim.g.node_host_prog
  local host_cmd = host:match("%.js$") and ("node " .. vim.fn.shellescape(host) .. " --version 2>/dev/null") or (host .. " --version 2>/dev/null")
  local version_output = vim.fn.system(host_cmd)
  if vim.v.shell_error ~= 0 then
    print("Node host: Error getting version for " .. host)
    return
  end

  local version = version_output:gsub("\n", ""):gsub("v", "")
  print("Nvim node host: " .. host .. " (v" .. version .. ")")
